...
function registerRoomSocket(io) {
  io.on("connection", (socket) => {
    socket.on("join-room", ({ roomId: rawId, videoUrl }, reply = () => {}) => {
      ...
      if (!room) {
        ...
        room = rooms.set({ id, hostId: socket.id, members: new Map(), state: { videoUrl, playing: false, position: 0, changedAt: now(), version: 1 } });
      }
      socket.join(id);
    ...
    socket.on("sync-request", () => { const room = rooms.get(socket.data.roomId); if (room) socket.emit("room-state", snapshot(room, socket.id)); });
